 ...
 */
export const parseRupeesToPaise = (value: string | null | undefined): number | null => {
 ...
 */
export const buildCacheKey = (query: string, pincode?: string): string => {
 ...
 */
export const normaliseManufacturerName = (name: string | null | undefined): string => {
  if (!name) return '';
  ...
  if (norm.includes('cipla')) return 'cipla';
  if (norm.includes('sun pharm') || norm.includes('sun pharma')) return 'sun pharma';
  if (norm.includes('dr. reddy') || norm.includes("dr reddy's") || norm.includes('dr reddy')) return 'dr reddys';
  if (norm.includes('torrent')) return 'torrent pharma';
  if (norm.includes('alkem')) return 'alkem';
  if (norm.includes('lupin')) return 'lupin';
  if (norm.includes('mankind')) return 'mankind';
  if (norm.includes('zydus') || norm.includes('cadila')) return 'zydus';
  if (norm.includes('abbott')) return 'abbott';
  if (norm.includes('gsk') || norm.includes('glaxosmithkline')) return 'gsk';
  if (norm.includes('sanofi')) return 'sanofi';
  if (norm.includes('pfizer')) return 'pfizer';
  if (norm.includes('biochem')) return 'biochem';
  if (norm.includes('micro lab')) return 'micro labs';
  if (norm.includes('intas')) return 'intas';
  if (norm.includes('glenmark')) return 'glenmark';
  if (norm.includes('ipca')) return 'ipca';
  if (norm.includes('apex')) return 'apex';
  if (norm.includes('usv')) return 'usv';
  if (norm.includes('alembic')) return 'alembic';
  if (norm.includes('aristo')) return 'aristo';
  if (norm.includes('macleod')) return 'macleods';
  if (norm.includes('hetero')) return 'hetero';
  if (norm.includes('jb chem') || norm.includes('j.b. chem') || norm.includes('j b chem')) return 'jb chemicals';
  if (norm.includes('systopic')) return 'systopic';
  if (norm.includes('blue cross')) return 'blue cross';
 ...
 */
export const stripFormDescriptors = (query: string): string =>
  query
 ...
 */
export const calculatePerUnitPrice = (
